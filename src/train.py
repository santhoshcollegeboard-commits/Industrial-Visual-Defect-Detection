"""
Training pipeline for Industrial Defect Detection.
Includes CLAHE contrast enhancement, AdamW optimization, validation checkpointing,
and convergence curve visualization.
"""
...
def plot_defect_training_curves(history: dict, save_path: str):
    ...
    ax1.set_title('Cross-Entropy Defect Loss vs Epochs', fontsize=13, fontweight='bold')
    ...
    ax2.set_title('Defect Classification Accuracy (%) vs Epochs', fontsize=13, fontweight='bold')
    ...
    plt.close()
    print(f"Industrial training curves saved to: {save_path}")
...
def train_defect_model(
    ...
    print(f"Executing Industrial Training on device: {device}")

    train_loader, val_loader, test_loader, classes = get_industrial_dataloaders(
        batch_size=batch_size, apply_clahe=True
    )
    ...
    print(f"Starting Industrial Defect Training for {epochs} Epochs on {len(classes)} classes...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_path = os.path.join(checkpoint_dir, "industrial_defect_best.pth")
            torch.save({
                'epoch': epoch,
    ...
    plot_defect_training_curves(history, plot_path)
...
if __name__ == '__main__':
    ...
    train_defect_model(epochs=args.epochs, batch_size=args.batch_size, lr=args.lr)
